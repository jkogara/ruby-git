module Git
  ...
  class Status
    ...
    def fetch_untracked
      ...
      all_files = ignore + @files.keys
      exclude_set = all_files.to_set


      Dir.chdir(@base.dir.path) do
        Dir.glob('**/*', File::FNM_DOTMATCH).reject { |f| f =~ %r{^.git\/.+}  }.each do |file|
          next if exclude_set.include?(file) || File.directory?(file)
